...
jQuery(document).ready(function($) {
    ...
    $(document).on('click', '#kodyt-btn-apply-coupon', function(e) {
        ...
        $.post(params.ajax_url, {
            action: 'kodyt_apply_checkout_coupon', security: params.checkout_nonce, coupon_code: code
        }, (response) => {
            $(this).prop('disabled', false).text('Apply');
            if (response.success) {
                feedback.css('color', '#10b981').text(response.data.message || 'Coupon added!').show();
                ...
                feedback.css('color', '#ef4444').text(response.data.message || 'Invalid coupon.').show();
            }
        }, 'json');
    });
    ...
    $(document).on('click', '.kodyt-remove-coupon-link', function(e) {
        ...
        $.post(params.ajax_url, { 
            action: 'kodyt_remove_checkout_coupon', security: params.checkout_nonce, coupon_code: code 
        }, function(response) {
            if (response.success) {
                ...
                feedback.css('color', '#64748b').text('Coupon removed.').show().delay(2000).fadeOut();
            }
        }, 'json');
    });
});
