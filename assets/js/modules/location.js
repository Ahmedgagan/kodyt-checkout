...
jQuery(document).ready(function($) {
    ...
    setupAutocompleteWatcher('#kodyt_shipping_autocomplete', '#kodyt_shipping_suggestions', 'shipping');
    setupAutocompleteWatcher('#kodyt_billing_autocomplete', '#kodyt_billing_suggestions', 'billing');
    ...
    function setupAutocompleteWatcher(inputSelector, boxSelector, prefix) {
        $(document).on('input', inputSelector, function() {
            ...
            debounceTimer = setTimeout(function() {
                $.post(params.ajax_url, {
                    action: 'kodyt_proxy_autocomplete', security: params.checkout_nonce, q: query
                }, function(response) {
        ...
        $(document).on('click', boxSelector + ' .kodyt-suggestion-item', function() {
            ...
            $.post(params.ajax_url, {
                action: 'kodyt_proxy_validate_address', security: params.checkout_nonce, place_id: selectedId, text: selectedText
            }, function(response) {
                if (response.success) {
                    let addr = response.data?.address || response.data;
                    if (addr.street) $(inputSelector).val(addr.street);
                    if (addr.house_number || addr.building_number) $(`#kodyt_${prefix}_house_number`).val(addr.house_number || addr.building_number);
                    if (addr.city) $(`#kodyt_${prefix}_city`).val(addr.city);
                    let postcode = addr.postcode || addr.zip || addr.postal_code || '';
                    if (postcode) $(`#kodyt_${prefix}_postcode`).val(postcode);
                    if (addr.country_code || addr.country) $(`#kodyt_${prefix}_country`).val(addr.country_code || addr.country);
                }
            }, 'json');
        });
    }
