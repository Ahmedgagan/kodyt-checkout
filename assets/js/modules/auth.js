...
jQuery(document).ready(function($) {
    ...
    $(document).on('click', '#kodyt-profile-btn-trigger-action', function(e) {
        ...
        let dialCode = window.kodytProfileItiInstance.getSelectedCountryData().dialCode || "";
        ...
        $.post(params.ajax_url, {
            ...
            phone: rawNumber, country_code: dialCode, otp_context: 'profile_change'
        }, function(response) {
            if (response && response.success === true) {
                $('#kodyt-profile-modal-overlay').css('display', 'flex');
                $('#kodyt_profile_modal_otp').val('').focus();
            } else {
                alert('Error: ' + (response.data?.message || 'Failed to dispatch verification code.'));
            }
    ...
    $(document).on('click', '#kodyt-profile-modal-btn-verify', function() {
        ...
        let dialCode = window.kodytProfileItiInstance ? window.kodytProfileItiInstance.getSelectedCountryData().dialCode : "";

        $.post(params.ajax_url, {
            action: 'kodyt_profile_verify_new_phone', security: params.checkout_nonce,
            phone: targetPendingNewPhone, country_code: dialCode, otp: token
        }, function(response) {
            if (response && response.success === true) {
                ...
                alert(response.data?.message || 'The validation code entered is invalid or expired.');
            }
    ...
    $(document).on('click', '#kodyt-btn-send-otp', function() {
        ...
        let phoneNum = activeInputTarget.length ? activeInputTarget.val().trim() : $('#kodyt_auth_phone').val().trim();
        ...
        $.post(params.ajax_url, {
            ...
            if (response && response.success === true) {
                ...
                alert('OTP Error: ' + (response.data?.message || 'Failed dispatch loop.'));
    ...
    $(document).on('click', '#kodyt-btn-verify-otp', function() {
        ...
        $.post(params.ajax_url, {
            ...
            if (response && response.success === true) {
                ...
                if (response.data && response.data.addresses && response.data.addresses.shipping) {
                ...
                $('html, body').animate({ scrollTop: $("#kodyt-step-shipping").offset().top - 40 }, 600);
                setTimeout(function() { $('#kodyt_shipping_autocomplete').focus(); }, 650);
                return;
            }
            
            let errorMsg = response.data?.message || response.messages || 'OTP mismatch. Please try again.';
    ...
    $(document).on('click', '#kodyt-account-btn-verify-otp', function() {
        ...
        let dialCode = window.kodytAccountItiInstance ? window.kodytAccountItiInstance.getSelectedCountryData().dialCode : "";

        $.post(params.ajax_url, {
            action: 'kodyt_account_otp_login', security: params.checkout_nonce,
            phone: phoneNum, country_code: dialCode, otp: code
        }, function(response) {
            if (response && response.success === true) {
